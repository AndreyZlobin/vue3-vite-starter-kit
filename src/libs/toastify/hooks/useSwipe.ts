...
const useSwipe = (
  ...
  const isMouseEvent = (event: MouseEvent | TouchEvent): boolean => event instanceof MouseEvent;

  const swipeHandler = (event: MouseEvent | TouchEvent) => {
    if (swipeClose === false || !swipeStart.value) return;

    if (isMouseEvent(event)) {
      swipedDiff.value = (swipeStart.value as MouseEvent).clientX - (event as MouseEvent).clientX;
    } else {
      swipedDiff.value =
